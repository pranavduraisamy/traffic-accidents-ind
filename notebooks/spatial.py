...
def _(compare_switch, df_options1, df_radio1, mo):
    if compare_switch.value:
        df_options2 = dict(
            filter(lambda i: i[1] != df_radio1.value, df_options1.items())
        )
        df_radio2 = mo.ui.radio(options=df_options2, inline=False)
        df_stack = mo.hstack(
            [
                mo.vstack([mo.md("### View A:"), df_radio1]),
                mo.vstack([mo.md("### View B:"), df_radio2]),
            ],
...
def _(requests):
    geo = requests.get(
        "https://raw.githubusercontent.com/pranavduraisamy/traffic-accidents-ind/refs/heads/main/notebooks/map/india.geojson"
    ).json()
    return (geo,)
...
def _(df_radio1, pd):
    df1 = pd.read_csv(
        "https://raw.githubusercontent.com/pranavduraisamy/traffic-accidents-ind/refs/heads/main/notebooks/data/"
        + df_radio1.value
    )
    return (df1,)
...
def _(autoplay_switch, mo):
    if autoplay_switch.value:
        timer = mo.ui.refresh(
            [3, 5, 7, 10, 15, 20], default_interval=7, label="Refresh in"
        )
    else:
        timer = None
    timer
...
def _(
    ...
    if compare_switch.value:
        ...
        plot_stack = mo.hstack(
            [
                plot_basic(
                    df=df1,
                    geo=geo,
                    df_radio_value=df_radio1.value,
                    year_value=get_year(),
                    compare_value=True,
                ),
                plot_basic(
                    df=df2,
                    geo=geo,
                    df_radio_value=df_radio2.value,
                    year_value=get_year(),
                    compare_value=True,
                ),
            ],
...
def _(df_radio1, hv, mo):
    def plot_basic(df, geo, df_radio_value, year_value, compare_value):
        ...
        map_plot = hv.Polygons(
            polygons, ["x", "y"], [("state", "State"), ("parameter", param_name[0])]
        )
        map_plot.opts(
            data_aspect=1,
            ...
            toolbar="above",
            tools=["hover"],
            color=hv.dim(param_name[0]),
            cmap="RdYlGn_r",
            clabel=param_name[0],
            title="Distribution of " + param_name[0] + " across states",
        )

        text_plot = hv.Text(92, 36, str(int(year_value))).opts(
            text_font_size=text_font_size, text_color="lightgray"
        )
            ...
            plot_stack = mo.hstack(
                [
                    plot_basic(
                        df=df1,
                        geo=geo,
                        df_radio_value=df_radio_value1,
                    ...
                    plot_basic(
                        df=df2,
                        geo=geo,
                        df_radio_value=df_radio_value2,
                        year_value=year_value,
                        compare_value=True,
                    ),
                ],
